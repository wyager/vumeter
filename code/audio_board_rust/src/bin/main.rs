...
fn main() -> ! {
    ...
    let mut switch = switch::Switch::initialize(&mut gpio2, pins.p12, pins.p11, pins.p10, pins.p9).unwrap();
    let mut _pwr = switch::Pwr::initialize(&mut gpio2, pins.p6).unwrap();
    let mut _sleep_pin = switch::SleepPin::initialize(&mut gpio2, pins.p7).unwrap();

    // This configures the LED as a GPIO output.
    let led = board::led(&mut gpio2, pins.p13);

    switch::Pwr::set(&mut gpio2, pins.p6, true);
    ...
    let mut counter_spdif: u32 = 0;
    let mut counter_uart: u32 = 0;
    let mut counter_timer: u32 = 0;
    let mut counter_switch: u32 = 0;
    loop {
        ...
        let switch = switch.check(&mut gpio2, pins.p12, pins.p11, pins.p10, pins.p9).map(Event::Switch);
        ...
        let _ = display.write_fmt(format_args!("Ctr: {}", counter));

        if !timer.is_none() {
            counter_timer = counter_timer.wrapping_add(1);
        }
        else {
            counter_timer = 0;
        }
        ...
        let _ = display.set_column(1);
        let _ = display.write_fmt(format_args!("ctr_timer: {}", counter_timer));

        if spdif.is_none() {
            counter_spdif = 0;
        }
        else {
            counter_spdif = counter_spdif.wrapping_add(1);
        }
        let _ = display.write_str(unsafe { core::str::from_utf8_unchecked(&[10]) });
        let _ = display.set_column(1);
        let _ = display.write_fmt(format_args!("ctr_spdif: {}", counter_spdif));

        if switch.is_none() && spdif.is_none() && timer.is_none() && uart.is_none() {
            switch::SleepPin::set(&mut gpio2, pins.p7, true);
            wfi();
            led.toggle();
            switch::SleepPin::set(&mut gpio2, pins.p7, false);
        } else {
        ...
        let _ = display.clear().unwrap();
        counter_uart = counter_uart.wrapping_add(1);
    }
}
